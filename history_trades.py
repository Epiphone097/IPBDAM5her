...
def reset_wallet(con):
    ...
    cur.execute(f'insert into wallet values ("1", 10000, {currency.convert_currency("EUR", "USD", "10000")})')
    con.commit()
...
def onnodige_tijden_verwijderen(con):
    ...
    for row in historydata:
        ...
        print(tijd)
        if tijd.hour < 9 or tijd.hour > 20:
            print("delete rij")
    ...
    for i in range(0, candlesLen):
        ...
        if candles['date_time'][i].hour < 21:
            ...
            if (signal == 1 or signal == 2) and pattern:
                ...
                # gemiddelde usd prijs tussen 2019-2021
                trade['usd_value'] = trade['eur_value'] * 1.18

                trade['doge_value'] = (trade['usd_value'] / candles['close'][i])
                trade['eur_value'] = round(trade['eur_value'], 2)
                trade['usd_value'] = round(trade['usd_value'], 2)
                trade['doge_value'] = round(trade['doge_value'], 10)
                if signal == 1:
                    cur.execute(
                        f"UPDATE wallet "
                        f"SET euro = euro - {trade['eur_value']}, "
                        f"usd = usd - {trade['usd_value']}"
                    )
                    cur.execute(
                        f"INSERT INTO trade (trade_id, original, pattern, eur_value,"
                        f"usd_value, doge_value, timestamp, taker_side, wallet_eur_value) "
                        f"VALUES ("
                        f"'{trade_id}', "
                        f"1, "
                        f"'{pattern}', "
                ...
                trade2['wallet_eur_value'] = wallet2[0]['eur_value']
                trade2['taker_side'] = 0
                trade2['original'] = 0
                trade2['doge_value'] = (trade['doge_value'] * (candles['close'][i+1]/candles['close'][i]))
                # Convert €100,- to USD and DOGE
                trade2['usd_value'] = trade['doge_value'] * candles['close'][i+1]
                #van dollar weer terug naar euro
                trade2['eur_value'] = trade2['usd_value'] / 1.18
                trade['eur_value'] = round(trade['eur_value'], 2)
                trade['usd_value'] = round(trade['usd_value'], 2)
                trade['doge_value'] = round(trade['doge_value'], 10)
                #selling coins
                ...
                cur.execute(
                    f"UPDATE wallet "
                    f"SET euro = euro + {trade2['eur_value']}, "
                    f"usd = usd + {trade2['usd_value']}"
                )
                new_wallet_eur = cur.execute("select euro from wallet").fetchall()[0][0]
                cur.execute(
